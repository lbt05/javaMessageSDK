...
public class ConversationAckPacket extends PeerBasedCommandPacket {
  ...
  public static ConversationAckPacket getConversationAckPacket(String peerId,
    ...
    cap.setAppId(InternalConfigurationController.globalInstance().getAppConfiguration()
        .getApplicationId());
  ...
  /**
   * 根据 timestamps 获取 ack 的 Packet 用于离线消息仅返回 count 时通知 server 该 timestamps 以前的消息均已收到 一般情况下只需要发送最大的
   * timestamp 即可，不需要发送全部
   * 
   * @param peerId
